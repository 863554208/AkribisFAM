...
namespace AkribisFAM.WorkStation
{
    public class AkrAction
    {
        ...
        public int WaitHomingFinished(GlobalManager.AxisName axisName, CancellationToken token, int timeout = 30000)
        {
            int agmIndex = (int)axisName / 8;
            int axisRefNum = (int)axisName % 8;
            DateTime now = DateTime.Now;
...
/// <summary>
        ...
        public int HomeModule(Modules module, CancellationToken token)
        {
            ...
            switch (module)
            {
                case Modules.LaiLiao:
                    ...
                    if (Move(AxisName.LSX, 0, (int)AxisSpeed.LSX, (int)AxisAcc.LSX, (int)AxisAcc.LSX) != 0) return -1;
                    if (Move(AxisName.LSY, 0, (int)AxisSpeed.LSY, (int)AxisAcc.LSY, (int)AxisAcc.LSY) != 0) return -1;

                    // WAIT MOVE TO FINISH
                    if (WaitHomingFinished(AxisName.LSX, token) != 0) return -1;
                    if (WaitHomingFinished(AxisName.LSY, token) != 0) return -1;

                    break;
                case Modules.ZuZhuang:
                    // MOVE Z HARDSTOP FIRST FOR SAFETY
                    if (HomeToHardStop(AxisName.PICK1_Z, token, 20000, false) != 0) return -1;
                    if (HomeToHardStop(AxisName.PICK2_Z, token, 20000, false) != 0) return -1;
                    ...
                    // HOME XY
                    if (Home_v2(AxisName.FSX, token, false) != 0) return -1;
                    if (Home_v2(AxisName.FSY, token, false) != 0) return -1;
                    ...
                    // HOME Z
                    if (Home_v2(AxisName.PICK1_Z, token, false) != 0) return -1;
                    if (Home_v2(AxisName.PICK1_T, token, false) != 0) return -1;
                    if (Home_v2(AxisName.PICK2_Z, token, false) != 0) return -1;
                    if (Home_v2(AxisName.PICK2_T, token, false) != 0) return -1;
                    //if (Home(AxisName.PICK3_Z, false) != 0) return -1;
                ...
                case Modules.FuJian:
                    ...
                    if (Move(AxisName.PRZ, 0, (int)AxisSpeed.PRZ, (int)AxisAcc.PRZ, (int)AxisAcc.PRZ) != 0) return -1;

                    // WAIT MOVE/HOMING TO FINISH
                    if (WaitHomingFinished(AxisName.PRZ, token) != 0) return -1;

                    // HOME XY
                    if (Home_v2(AxisName.PRX, token, false) != 0) return -1;
                    if (Home_v2(AxisName.PRY, token, false) != 0) return -1;
        ...
        public int Home_v2(AxisName axisName, CancellationToken token, bool waitHomeComplete = true)
        {
            int agmIndex = (int)axisName / 8;
            int axisRefNum = (int)axisName % 8;
            AAMotionAPI.Home(AAmotionFAM.AGM800.Current.controller[agmIndex],
        ...
        public int HomeToHardStop(AxisName axisName, CancellationToken token, int timeout = 30000, bool waitHomeComplete = true)
        {
            int agmIndex = (int)axisName / 8;
            int axisRefNum = (int)axisName % 8;
            AAMotionAPI.HomeToHardStop(AAmotionFAM.AGM800.Current.controller[agmIndex],
                                       GlobalManager.Current.GetAxisRefFromInteger(axisRefNum),
                                       4000,
                                       -10000,
                                       100000,
            ...
            {
                // Wait for homing to finish
                return WaitHomingFinished(axisName, token, timeout);
            }
            return 0;
        }
